...
// Поиск индекса, соответствующего вольтажу. Индекс соответствует проценту оставшегося заряда
  ...
  int idx = 0;
  while (true) {
    if (Volt < vs[idx+1]) {mVolt = Volt - vs[idx]; ChargeLevel = idx * 5 + round(mVolt /((vs[idx+1] - vs[idx]) / 5 )); break;}	// Получение % оставшегося заряда
    else {idx++;}
                 }
...
void DspCore::_clockTime(){
 ...
 // if(!config.isScreensaver) drawFastHLine(width()-clockRightSpace-CHARWIDTH*3*2-18, clockTop-clockTimeHeight+29, 44, config.theme.div);              /*divider hor*/
  ...
  _dateleft = _timeleft + 35+(width() - _timeleft - _datewidth - clockRightSpace) / 2;
