 ...
 *  Updated on: Dec 29,2024 (Maleksm)
...
void Audio::processLocalFile() {
    ...
    if(!m_f_stream) {
        if(m_codec == CODEC_OGG) { 					// log_i("determine correct codec here");
            ...
            if     (codec == CODEC_FLAC)   { m_codec = codec; AUDIO_INFO("format is flac"); return; }
            else if(codec == CODEC_OPUS)   { m_codec = codec; AUDIO_INFO("format is opus"); return; }
            else if(codec == CODEC_VORBIS) { m_codec = codec; AUDIO_INFO("format is vorbis"); return; }
...
//    const uint16_t  maxFrameSize = InBuff.getMaxBlockSize();    // every mp3/aac frame is not bigger
    ...
    if(f_chunkFinished) {
        if(m_f_psramFound) {
            if(InBuff.bufferFilled() < 60000) { f_chunkFinished = false; m_f_continue = true; }
        }
        else {
            f_chunkFinished = false;
            m_f_continue = true;
...
const char* Audio::m3u8redirection(uint8_t* codec){
 ...
 //   if((!endsWith(m_playlistContent[choosenLine], "m3u8") && indexOf(m_playlistContent[choosenLine], "m3u8?") == -1)) {
        ...
        // int pos = indexOf(m_playlistContent[choosenLine - 1], "CODEC=\"mp4a", 18);
    //     if(pos < 0) { // not found
...
//    strcpy(m_lastHost, h_host);
    ...
    if(res) {
        ...
        if(endsWith(h_host, ".m3u8")) {
            m_expectedPlsFmt = FORMAT_M3U8; if(audio_lasthost) audio_lasthost(host); }

        m_dataMode = HTTP_RESPONSE_HEADER; 				// Handle header
        m_streamType = ST_WEBSTREAM;
    }
    else {
        AUDIO_ERROR("Request %s failed!", host);
        m_f_running = false;
        if(audio_showstation) audio_showstation("");
...
bool Audio::connecttospeech(const char* speech, const char* lang) {
    ...
    if(urlStr) { free(urlStr); urlStr = NULL; }
