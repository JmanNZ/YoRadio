 ...
 *
 *  Version 3.0.12f
 *  Updated on: Jul 29.2024, Jul 30.2024 (Maleksm)
...
//#define AUDIO_ERROR(...) { sprintf(m_ibuff, __VA_ARGS__); if(audio_info) audio_info(m_ibuff); }
...
void Audio::setBufsize(int rambuf_sz, int psrambuf_sz) {
    if(InBuff.isInitialized()) { log_e("Audio::setBufsize must not be called after audio is initialized");
        return;
    }
...
//        AUDIO_INFO("Hostaddress is empty");
        AUDIO_ERROR("Hostaddress is empty");
        stopSong();
//        xSemaphoreGiveRecursive(mutex_audio);
        xSemaphoreGive(mutex_playAudioData);
        return false;
...
//        AUDIO_INFO("Hostaddress is too long");
        AUDIO_ERROR("Hostaddress is too long");
        stopSong();
//        xSemaphoreGiveRecursive(mutex_audio);
        xSemaphoreGive(mutex_playAudioData);
        return false;
...
bool Audio::httpPrint(const char* host) {
    ...
    if(!_client->connected()) {
        ...
        if(!_client->connect(hostwoext, port)) { log_e("connection lost");
            stopSong();
            return false;
        }
    }
...
// clang-format on
//****************************************************************************************
//bool Audio::connecttoSD(String sdfile, uint32_t fileStartPos) {
//    return connecttoFS(SD, sdfile.c_str(), (int32_t) fileStartPos); }

bool Audio::connecttoSD(const char* path, uint32_t fileStartPos) {
    return connecttoFS(SD, path, (int32_t)fileStartPos); }
//****************************************************************************************
...
//        xSemaphoreGiveRecursive(mutex_audio);
    ...
    if(endsWith(afn, ".mp3")) { m_codec = CODEC_MP3; audio_info("format is mp3"); }
    if(endsWith(afn, ".m4a")) { m_codec = CODEC_M4A; audio_info("format is aac"); }
    if(endsWith(afn, ".aac")) { m_codec = CODEC_AAC; audio_info("format is aac"); }
    if(endsWith(afn, ".wav")) { m_codec = CODEC_WAV; audio_info("format is wav"); }
    if(endsWith(afn, ".flac")) { m_codec = CODEC_FLAC; audio_info("format is flac"); }
    if(endsWith(afn, ".opus")) { m_codec = CODEC_OPUS; audio_info("format is opus"); }
    if(endsWith(afn, ".ogg")) { m_codec = CODEC_OGG; audio_info("format is ogg"); }
    if(endsWith(afn, ".oga")) { m_codec = CODEC_OGG; audio_info("format is ogg"); }
		 // m_codec is by default CODEC_NONE
...
void Audio::urlencode(char* buff, uint16_t buffLen, bool spacesOnly) {
    ...
    for(int i = 0; i < len; i++) {
        ...
        if(j == buffLen - 1) { log_e("out of memory");
            break;
        }
    }
...
bool Audio::latinToUTF8(char* buff, size_t bufflen, bool UTF8check) {
    ...
    char* iso8859_1 = NULL;
    if(m_f_psramFound){
        char* iso = (char*)ps_malloc(strlen(buff) + 1);
        char* iso8859_1 = strcpy(iso, buff);
        } else char* iso8859_1 = strdup(buff);
...
bool Audio::readPlayListData() {
    ...
    while(true) { // outer while
        ...
        if(strlen(pl) > 0) {
             if(m_f_psramFound) {
                char* s = (char*)ps_malloc(strlen(pl) + 1);
                m_playlistContent.push_back(strcpy(s, pl));
             } else m_playlistContent.push_back(strdup(pl));
        }
...
void Audio::processLocalFile() {
    ...
    uint32_t bytesAddedToBuffer = audiofile.read(InBuff.getWritePtr(), availableBytes);
    ...
    if(!m_f_stream) {
        if(m_codec == CODEC_OGG) { 					// log_i("determine correct codec here");
            ...
            if(codec == CODEC_FLAC) { m_codec = CODEC_FLAC; audio_info("format is flac"); initializeDecoder();  return; }
            if(codec == CODEC_OPUS) { m_codec = CODEC_OPUS; audio_info("format is opus"); initializeDecoder(); return; }
            if(codec == CODEC_VORBIS) { m_codec = CODEC_VORBIS; audio_info("format is vorbis"); initializeDecoder(); return; }
            stopSong();
            return;
        }
    ...
    if(m_fileStartPos > 0) { setFilePos(m_fileStartPos); m_fileStartPos = -1; }
...
void Audio::processWebStream() {
    ...
    if(availableBytes) {
        availableBytes = min(availableBytes, (uint32_t)InBuff.writeSpace());
        uint32_t bytesAddedToBuffer = _client->read(InBuff.getWritePtr(), availableBytes);
...
//    const uint16_t  maxFrameSize = InBuff.getMaxBlockSize(); 	// every mp3/aac frame is not bigger
    ...
    if(f_chunkFinished) {
        if(m_f_psramFound) {
            if(InBuff.bufferFilled() < 150000) { f_chunkFinished = false; m_f_continue = true; }
        }
        else {
            f_chunkFinished = false;
            m_f_continue = true;
...
//        AUDIO_INFO("ContentType %s not supported", ct);
    ...
    switch(ct_val) {
        ...
        case CT_WAV:
            ...
            break;
        case CT_OGG:
            if(m_expectedCodec == CODEC_VORBIS) { m_codec = CODEC_VORBIS; audio_info("format is vorbis");}
            else if(m_expectedCodec == CODEC_OPUS) { m_codec = CODEC_OPUS; audio_info("format is opus"); }
            else if(m_expectedCodec == CODEC_FLAC) { m_codec = CODEC_FLAC; audio_info("format is flac"); }
            else { m_codec = CODEC_OGG; audio_info("format is ogg"); } // determine in first OGG packet -OPUS, VORBIS, FLAC
            break;
...
uint32_t Audio::getAudioFileDuration() {
    if(getDatamode() == AUDIO_LOCALFILE) { if(!m_audioDataSize) return 0; }
...
//    m_gain0 = map(gainLowPass, -16, 16, -6, 6);
//    m_gain1 = map(gainBandPass, -16, 16, -6, 6);
//    m_gain2 = map(gainHighPass, -16, 16, -6, 6);
...
//    else if(m_balance > 0)  l = m_balance * 16;
    ...
    m_limit_left = m_vol * l / 274;
    m_limit_right = m_vol * r / 274;
...
//    if(m_balance < 0) { step = step * abs(m_balance) * 16;
//        l = (uint8_t)(step);
//    }
//    if(m_balance > 0) { step = step * m_balance * 16;
//        r = (uint8_t)(step);
//    }

//    v[LEFTCHANNEL] = (sample[LEFTCHANNEL]  * (m_vol - l)) >> 8;
//    v[RIGHTCHANNEL]= (sample[RIGHTCHANNEL] * (m_vol - r)) >> 8;


    /* important: these multiplications must all be signed ints, or the result will be invalid */
//    if(m_bitsPerSample == 16){
//        sample[LEFTCHANNEL]  = ((sample[LEFTCHANNEL]  * (m_vol - r)) >> 8);
//        sample[RIGHTCHANNEL] = ((sample[RIGHTCHANNEL] * (m_vol - l)) >> 8);
//    }
    /* important: all samples are unsigned 8 bit, 128 is 0, +255 is max (+127), 0 is min (-128)  */
//    if(m_bitsPerSample == 8){
//        uint8_t* s = reinterpret_cast <uint8_t*>(sample);
//        s[LEFTCHANNEL]  = ((s[LEFTCHANNEL] * (m_vol - r)) >> 8);
//        s[RIGHTCHANNEL] = ((s[RIGHTCHANNEL] * (m_vol - l)) >> 8);
//    }
//}
//****************************************************************************************
...
void Audio::seek_m4a_ilst() {
    ...
    /* atom hierarchy (example)______________________________________________________________
